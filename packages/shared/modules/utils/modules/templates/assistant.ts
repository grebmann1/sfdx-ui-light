...
const generateItems = (variableName: string, content: string, role: string): string => {
...
export const instructionFormatted = (): string => {
    const instructions = [
...
const escapeApexString = (input: string): string => {
    if (!input) return '';
...
export const chat_template = (model: string, messages: ChatMessage[]): string => {

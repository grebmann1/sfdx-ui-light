import { store, SELECTORS, SOBJECT } from 'core/store';
...
const EXPRESSION_BOOLEAN = ['AND', 'OR'];
...
export default class SuggestionHandler {
    ...
    getExpressionBooleanAfter() {
        return [
            ...AFTER_FROM_STATEMENT.filter(value => value !== 'WHERE').map(value =>
                this.suggestionHandler.DEFAULT(value, false)
            ),
        ...
        return store
            .dispatch(
                SOBJECT.describeSObject({
                    ...
                    useToolingApi:
                        describe.nameMap[lowerCaseKey(this.parser.fromObject)]?.useToolingApi,
                })
            )
            .then(() => {
                ...
                const sobjectState = SELECTORS.sobject.selectById(
                    { sobject },
                    (this.parser.fromObject || '').toLowerCase()
                );
                return (sobjectState?.data?.childRelationships || [])
                    .map(rel => this.suggestionHandler.RELATIONSHIP(rel, isSnippet))
    ...
    getLookupFieldSuggestions = () => {
        ...
        return Promise.all(
            relation.map(rel => {
                ...
                const sobjectState = SELECTORS.sobject.selectById(
                    { sobject },
                    (currentReferenceTo || '').toLowerCase()
                );
                if (!sobjectState || !sobjectState.data) return; // Object no yet fully setup
                const fields = sobjectState.data.fields.filter(
                    field => field.relationshipName && field.relationshipName.toLowerCase() === rel
                );
                if (fields && fields.length > 0) {
                    ...
                    return store.dispatch(
                        SOBJECT.describeSObject({
                            ...
                            useToolingApi:
                                describe.nameMap[lowerCaseKey(currentReferenceTo)]?.useToolingApi,
                        })
                    );
                }
                return;
    ...
    getTypeFieldSuggestions = async (object, fieldData, withSeparator) => {
        ...
        const referenceTo = this.parser.isSubQuery
            ? this.parser.subquery.type === 'select'
                ? this.extractObjectFromChild()
                : this.parser.subquery.fromObject
            : object;
        ...
        switch (fieldInfo.type) {
            case 'picklist':
                return fieldInfo.picklistValues.map(item =>
                    this.suggestionHandler.PICKLIST(item, withSeparator)
                );
            case 'date':
            case 'datetime':
    ...
    extractObjectFromChild() {
        ...
        const childRelation = sobjectState.data.childRelationships.find(
            rel =>
                rel.relationshipName &&
                rel.relationshipName.toLowerCase() === this.parser.subquery.fromObject
        );
    ...
    getFields = async (referenceTo, withSeparator = true) => {
        ...
        return new Promise(async resolve => {
            ...
            const fields = [];
            (sobjectState?.data?.fields || []).forEach(field => {
    ...
    getSelectedFieldSuggestion() {
        return this.parser.parsedData.fields.map(field =>
            this.suggestionHandler.DEFAULT(field, true)
        );
    }
}

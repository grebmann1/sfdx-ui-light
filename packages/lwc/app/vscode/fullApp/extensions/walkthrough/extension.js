...
import { buildSalesforceExtensionConfig, EXTENSION_VERSION, EXTENSION_PUBLISHER } from '../salesforce/salesforceExtensionSupport.js';
import { registerSalesforceExtension, createCallbackDisposable } from '../shared.js';
...
export function buildWalkthroughMarkdown(orgContext) {
...
export async function register(vscodeBundle, { orgContext } = {}) {
            ...
            walkthroughs: [
                {
                    ...
                    description: 'Review your connected org, understand the limits of this lightweight workbench, and use the built-in agent effectively.',
    ...
    const WALKTHROUGH_EXTENSION_ASSETS = [
        {
            //sourcePath: '/libs/extensions/walkthrough/workbench-onboarding.md',
            targetPath: '/workspace/vscode/walkthrough/workbench-onboarding.md',
            content: 'data:text/markdown;base64,' + window.btoa(unescape(encodeURIComponent(buildWalkthroughMarkdown(nextOrgContext)))),
    ...
    return registerSalesforceExtension(
        ...
        async (vscode, { push }) => {
            console.log('entered walkthrough extension', vscode);
            if (!vscode?.commands) return;

            const walkthroughsService = await vscodeBundle.vscodeApi.getService(vscodeBundle.vscodeApi.IWalkthroughsService);
            walkthroughsService.registerWalkthrough(manifest.contributes.walkthroughs[0]);
            push(createCallbackDisposable(() => walkthroughsService.unregisterWalkthrough(manifest.contributes.walkthroughs[0])));

            vscode.commands.executeCommand('workbench.action.openWalkthrough', `${WALKTHROUGH_EXTENSION_ID}#open`);
            console.log('opened walkthrough');
        }
    );
}

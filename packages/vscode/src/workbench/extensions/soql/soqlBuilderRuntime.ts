...
const BUILDER_SOURCE_HTML ='/libs/extensions/salesforcedx-vscode-soql/dist/soql-builder-ui/index.html';
const BUILDER_SOURCE_JS ='/libs/extensions/salesforcedx-vscode-soql/dist/soql-builder-ui/app.js';
...
function transformBuilderHtml({
    ...
    nextHtml = nextHtml.replace('./app.js', BUILDER_SOURCE_JS);
    return nextHtml;
}
...
class SoqlBuilderEditorInstance {
    ...
    private async syncSObjectMetadata(sobjectName: string) {
        ...
        if (!schemaTools?.ensureSObjectDescribe || !conn?.instanceUrl || !conn?.accessToken) {
            console.log('no schemaTools or conn',schemaTools);

...
import { registerSoqlEditorCommands } from './soqlEditorCommands';
import { registerSoqlSchemaExplorer } from './soqlSchemaExplorer';
import { runAndShowSoqlQueryPlan } from './soqlQueryPlan';
import { executeSoqlQuery } from './soqlQueryRunner';
import { LanguageClientWrapper, type LanguageClientConfig } from '../../../languageClient/wrapper';
...
export async function registerUnifiedSoqlExtension(
    ...
    return registerSalesforceExtension(
        ...
        async (vscode, { push, vscodeBundle: bundle }) => {
            ...
            const extensionContext = context as Parameters<typeof createSoqlDataViewRuntime>[0]['context'];
            const soqlConnectionRuntime = connectionRuntime as Parameters<typeof registerSoqlBuilderRuntime>[0]['connectionRuntime'];
            ...
            try {
                console.log('[sf-soql-workbench] --> starting language client', languageClientConfig);
                const wrapper = new LanguageClientWrapper(languageClientConfig as unknown as LanguageClientConfig);
                await wrapper.start();
                push(wrapper);

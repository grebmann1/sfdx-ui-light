...
import {
    ...
    onWorkbenchAiBridgeConfigLoaded,
} from '../../bridge/bridgeConnection';
...
function buildBridgeModelInfoList() {
    const bridgeConfig = getWorkbenchAiBridgeConfig();
    if (!bridgeConfig?.models?.length) {
        return null;
    }
    return bridgeConfig.models.map(m => ({
        id: `${WORKBENCH_CHAT_FAMILY_PREFIX}${m.id}`,
        name: m.label,
...
export async function register(vscodeBundle) {
    ...
    // Re-advertise models when the bridge config arrives (async after handshake).
    disposables.push(
        onWorkbenchAiBridgeConfigLoaded(() => {
    ...
    disposables.push(
        vscodeApi.lm.registerLanguageModelChatProvider(WORKBENCH_CHAT_MODEL_VENDOR, {
            provideLanguageModelChatInformation() {
                return buildBridgeModelInfoList() ?? [bridge.createModelInfo()];
            },

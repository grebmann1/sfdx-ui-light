        ...
        contributes: {
            ...
            commands: [
                ...
                },
                {
                    ...
                    title: 'Salesforce: Deploy Current File',
                    icon: '$(cloud-upload)',
                },
                {
                ...
                },
                {
                    ...
                    title: 'Salesforce: Review and Deploy Changed Files',
                },
                {
                    ...
                    context.addDisposable?.(
                        vs.workspace.onDidOpenTextDocument(async doc => {
                            try {
                                ...
                                if (
                                    path.includes('/force-app/main/') &&
                                    path.includes('/lwc/')
                                ) {
                                    if (path.endsWith('.html') && doc.languageId !== 'html') {
                                        await vs.languages.setTextDocumentLanguage(doc, 'html');
                                    } else if (
                                        path.endsWith('.css') &&
                                        doc.languageId !== 'css'
                                    ) {
                                        await vs.languages.setTextDocumentLanguage(doc, 'css');
                                    } else if (
                                        path.endsWith('.js') &&
                                        doc.languageId !== 'javascript'
                                    ) {
                                        await vs.languages.setTextDocumentLanguage(
                                            doc,
                                            'javascript'
                                        );
                                    } else if (

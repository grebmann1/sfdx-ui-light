...
export function partitionChangedPathsForDeploy(paths, toolingMapItems, metadataApiItems = null) {
    const summary = {
        deployablePaths: [],
        metadataApiPaths: [],
        missingPaths: [],
    ...
    for (const path of Array.isArray(paths) ? paths : []) {
        ...
        if (entry) {
            ...
            } else if (!DEPLOYABLE_TOOLING_TYPES.has(entry.type)) {
                summary.unsupportedPaths.push(path);
            } else if (!isDeployableToolingEntry(entry)) {
                summary.missingPaths.push(path);
            } else {
                summary.deployablePaths.push(path);
            }
        } else if (metadataApiItems?.[path]) {
            summary.metadataApiPaths.push(path);
        } else {
            summary.missingPaths.push(path);
        }
    }

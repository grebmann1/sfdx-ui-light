...
export default class Overlay extends ToolkitElement {
    ...
    handleSearch = (e) => {
        runActionAfterTimeOut(e.detail.value, async (value) => {
    ...
    scrollToTop = () => {
        window.setTimeout(() => {
            if(this.template.querySelector('.scroll-container-header')){
                this.template.querySelector('.scroll-container-header').scrollTo({top: 0, behavior: 'auto'});
            }
        }, 100);
    };

    listenToContainerFocus = (event)=>  {
        if(this._cancelBlur || this.refs.container.contains(event.relatedTarget) && this.isOverlayDisplayed){
    ...
    search = async (searchTerm) => {
        ...
        // Setup Links
        if(true){ // this.viewerTab === TABS.QUICKLINK
            combinedResults.push(
        ...
        if (this.cachedData?.aura && this.checkCategory(TYPE.AURA)) {
            combinedResults.push(
                ...this.cachedData.aura
                    .filter(x => x.DeveloperName.toLowerCase().includes(lowerCaseTerm)|| isUndefinedOrNull(searchTerm))
                    .map(x => ({
                        id: x.Id,
                        type: TYPE.AURA,
    ...
    loadCachedData = async () => {
        ...
        const cacheExpiry = await window.defaultStore.getItem(`${CACHE_EXPIRY_KEY}-${this.currentDomain}`);
        ...
        if (!this._forceRefresh && cachedData && cacheExpiry && Date.now() < parseInt(cacheExpiry)) {
        ...
        try {
            ...
            // Fetch Apex Classes
            const apexClassQuery = this.connector.conn.query("SELECT Id,Name,NamespacePrefix,Status,ApiVersion FROM ApexClass WHERE NamespacePrefix = null");
            const apexClass = await apexClassQuery.run({ responseTarget:'Records',autoFetch : true, maxFetch : 100000 }) || [];

            // Fetch Apex Trigger
            const apexTriggerQuery = this.connector.conn.query("SELECT Id,Name,NamespacePrefix,Status,ApiVersion FROM ApexTrigger WHERE NamespacePrefix = null");
            const apexTrigger = await apexTriggerQuery.run({ responseTarget:'Records',autoFetch : true, maxFetch : 100000 }) || [];

            // Fetch Aura
            const auraQuery = this.connector.conn.query("SELECT Id,DeveloperName,NamespacePrefix,ApiVersion,MasterLabel FROM AuraDefinitionBundle WHERE NamespacePrefix = null");
            const aura = await auraQuery.run({ responseTarget:'Records',autoFetch : true, maxFetch : 100000 }) || [];

            // Fetch LWC
            const lwcQuery = this.connector.conn.tooling.query("SELECT Id,DeveloperName,NamespacePrefix,ApiVersion,MasterLabel FROM LightningComponentBundle WHERE NamespacePrefix = null");
            const lwc = await lwcQuery.run({ responseTarget:'Records',autoFetch : true, maxFetch : 100000 }) || [];
            ...
            const permSetsResult = await this.connector.conn.query('SELECT Id, Name FROM PermissionSet');
    ...
    get sldsPopupContainerClass(){
        return `slds-popup-container ${this.isFooterDisplayed?'':'slds-popup-container-no-footer'}`;
    }

    get sldsButtonGroupClass(){
        return `sf-toolkit slds-toolkit-button-group ${this.isOverlayDisplayed?'slds-is-active':''}`;
    }

    get virtualList(){
        // Best UX Improvement !!!!
        const _filteredList = this.formattedResults.filter( x =>

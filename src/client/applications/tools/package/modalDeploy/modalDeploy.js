...
export default class ModalDeploy extends LightningModal {
    ...
    run = () => {
        ...
        store.dispatch(PACKAGE.reduxSlice.actions.clearCurrentDeploymentJob());
        this.deploymentPromise = store.dispatch(PACKAGE.executePackageDeploy({
    ...
    get resultMessageDetails(){
        return this.resultProcessing?`Processing Time : ${this.resultProcessing}, `:''+`DeploymentId : ${this._response?.id} `;
    }

    get resultProcessing(){
        if(this._response?.startDate && this._response?.completedDate){
            const diff = (new moment(this._response?.completedDate)).diff(new moment(this._response?.startDate));

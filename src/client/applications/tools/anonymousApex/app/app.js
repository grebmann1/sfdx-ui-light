...
export default class App extends ToolkitElement {
    ...
    connectedCallback(){
        ...
        store.dispatch((dispatch, getState) => {
            dispatch(APEX.reduxSlice.actions.loadCacheSettings({
                alias:this.alias,
                apexFiles:getState().apexFiles
            }));
            dispatch(DOCUMENT.reduxSlices.APEXFILE.actions.loadFromStorage({
                alias:this.alias
            }));
            dispatch(APEX.reduxSlice.actions.initTabs({
                apexFiles:getState().apexFiles
            }));
        })
    ...
    storeChange({ apex,recents,apexFiles,application }) {
        ...
        if(apex.currentTab && this._hasRendered){
            ...
            if(this.currentTab.fileId != this.currentFile?.id){
                this.currentFile = SELECTORS.apexFiles.selectById({apexFiles},lowerCaseKey(this.currentTab.fileId));
            }
        }
    ...
    debug_handleChange = (e) => {
        ...
        store.dispatch(APEX.reduxSlice.actions.updateDebug({
    ...
    handleError = (data) => {
        ...
        this.error_message = data.exceptionMessage?`${data.exceptionMessage} | ${data.exceptionStackTrace}`:data.compileProblem;
    }
    ...
    handleSelectItem = (e) => {
        ...
        if(category === CATEGORY_STORAGE.SAVED){
            ...
            if(existingTab){
                ...
                store.dispatch(APEX.reduxSlice.actions.addTab({
                    tab:{
                        id:guid(),
                        body:content,
                        fileId:id
    ...
    handleCloseTab = async (e) => {
        ...
        const params = {variant: 'headerless',message: 'This script has unsaved changes ! Do you want delete it ?'}

...
import { decodeError, isNotUndefinedOrNull, isUndefinedOrNull, guid, isEmpty, checkIfPresent, classSet, lowerCaseKey, isChromeExtension } from 'shared/utils';
...
export default class App extends ToolkitElement {
    ...
    storeChange({ application, platformEvent, recents }) {
        ...
        if (this.currentChannelName) {
            const subscriptionState = SELECTORS.platformEvents.selectById({ platformEvent }, this.currentChannelName);
        ...
        this.subscribedChannels = this.formatChannels(SELECTORS.platformEvents.selectAll({ platformEvent }));
    ...
    toggle_recent = () => {
        store.dispatch(EVENT.reduxSlice.actions.updateRecentPanel({
    ...
    handleEventSelection = (e) => {
        ...
        try {
            store.dispatch(EVENT.reduxSlice.actions.updateReadStatusOnSpecificMessage({
    ...
    handleEmptyMessages = () => {
        // Reset message list
        this.selectedEventItem = null;
        store.dispatch(EVENT.reduxSlice.actions.cleanMessages({
            channel: this.currentChannelName
        }));
    }
    ...
    _subscribeChannel = (eventName, replayId) => {
        ...
        if (this.activeSubscriptions.hasOwnProperty(lowerCaseKey(eventName))) {
            ...
            store.dispatch(EVENT.reduxSlice.actions.updateChannel({
                value: eventName
            }));
        } else {
            try {
    ...
    fetchReplayId = (channel) => {
        ...
        const subscriptionState = SELECTORS.platformEvents.selectById({ platformEvent }, lowerCaseKey(channel));
    ...
    cometdSubscribe = async (eventName, replayId) => {
        ...
        // Verify ReplayId 
        const params = { theme: 'warning', label: 'Reprocessing all messages stored in the BUS', message: `Subscribing with the -2 option when a large number of event messages are stored can slow performance and consume your entire Publishing/Develivery allocation.` }
        if (_replayId <= -2 && !await LightningConfirm.open(params)) return;

        let { type, shortName } = this.extractInfoFromPath(eventName);

        store.dispatch(EVENT.reduxSlice.actions.createSubscription({
            channel: eventName,
            name: shortName,
            type,
        ...
        // Subscribing
        this.activeSubscriptions[_formattedEventName] = cometd.subscribe(eventName, async (message) => {

            const item = { id: message.data.event.replayId, receivedDate: Date.now(), content: message };
            store.dispatch(EVENT.reduxSlice.actions.upsertSubscriptionMessages({
                messages: [item],
        ...
        store.dispatch(EVENT.reduxSlice.actions.updateSubscriptionStatus({
            channel: _formattedEventName,
            status: STATUS_SUBSCRIBED
        }));
        store.dispatch(DOCUMENT.reduxSlices.RECENT.actions.savePlatformEvents({
            channel: eventName,
            alias: this.alias
        }));
    ...
    handleChannelDeletion = (name) => {
        ...
        if (_channelToDelete) {
            ...
            store.dispatch(EVENT.reduxSlice.actions.deleteSubscription({
                channel: name
            }));
    ...
    updateLastMessageFormatted = () => {
        this.lastMessageFormatted = this.currentChannel?.lastModifiedDate ? moment(this.currentChannel.lastModifiedDate).fromNow() : DEFAULT_LAST_MESSAGE;
    }
    ...
    formatCurrentSubscription = (state) => {
        ...
        return {
            ...state,
            formattedType: TYPE_MAPPING.hasOwnProperty(state.type) ? TYPE_MAPPING[state.type] : TYPE_MAPPING['default'],
            counter: state.messages.length,
            badgeClass: (state.status == STATUS_SUBSCRIBED ? 'slds-theme_success' : (state.status == STATUS_ERROR ? 'slds-theme_error' : 'slds-badge') + ' slds-float_right')
        }
    }
    ...
    get isSubscribeDisabled() {
        return isUndefinedOrNull(this.lookup_selectedEvents) || this.lookup_selectedEvents.length == 0;
    }

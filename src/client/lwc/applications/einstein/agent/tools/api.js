...
const apiAgentTools = [
    tool({
        name: 'getTabs',
        description: `Get all API tabs currently open in the API editor. Returns an array of tab objects representing the current open API tabs. Use this to list or inspect all open tabs.`,
        parameters: z.object({}),
        execute: getTabs,
    }),
    tool({
        name: 'selectTab',
        description: `Select a tab by its ID in the API editor. The tabId must correspond to an existing tab. If the tabId does not exist, the selection will fail.`,
        parameters: z.object({ tabId: z.string().describe('Tab ID to select') }),
        execute: selectTab,
    }),
    tool({
        name: 'upsertTab',
        description: `Add or update a tab in the API editor. If the tab.id matches an existing tab, that tab will be updated. If the tab.id is new, a new tab will be created. When creating a new tab, ensure the id is unique and not reused from an existing tab. Reusing an existing tab id for a new tab is not allowed and will result in updating the existing tab instead.`,
    ...
    tool({
        ...
        description: `Get the list of recent API calls. Returns an array of recent API call objects, ordered from most recent to least recent.`,
        parameters: z.object({}),
        execute: getRecentApiCalls,
    }),
    tool({
        name: 'getSavedApiScripts',
        description: `Get the list of saved API scripts. Only scripts that are global or associated with an alias are returned. Use this to retrieve reusable API scripts.`,
        parameters: z.object({}),
        execute: getSavedApiScripts,
    }),
    tool({
        name: 'getOpenAPISavedScripts',
        description: `Get the list of saved OpenAPI schema files. Returns all OpenAPI schema files currently available in the editor.`,
        parameters: z.object({}),
        execute: getOpenAPISavedScripts,
    }),
    tool({
        name: 'getOpenAPIMethodForScript',
        description: `Get the OpenAPI method definition for a specific script and HTTP method. Provide a valid scriptId and HTTP method (GET, POST, etc.). Returns the path, method, and operation details if found, otherwise returns null.`,
    ...
    tool({
        name: 'updateBody',
        description: `Update the body for a specific tab. The tabId must correspond to an existing tab. The body should be a string representing the request payload.`,
        parameters: z.object({ tabId: z.string().describe('Tab ID'), body: z.string().describe('Request body') }),
        execute: updateBody,
    }),
    tool({
        name: 'updateHeader',
        description: `Update the header for a specific tab. The tabId must correspond to an existing tab. The header should be a string representing the request headers.`,
        parameters: z.object({ tabId: z.string().describe('Tab ID'), header: z.string().describe('Request header') }),
        execute: updateHeader,
    }),
    tool({
        name: 'updateVariable',
        description: `Update the global variables for the API editor. Variables should be provided as a JSON string. This will overwrite all existing variables.`,
        parameters: z.object({ variables: z.string().describe('Variables as JSON string') }),
        execute: updateVariable,
    }),
    tool({
        name: 'updateEndpoint',
        description: `Update the endpoint for a specific tab. The tabId must correspond to an existing tab. The endpoint should be a valid API endpoint string.`,
        parameters: z.object({ tabId: z.string().describe('Tab ID'), endpoint: z.string().describe('API endpoint') }),
        execute: updateEndpoint,
    }),
    tool({
        name: 'updateMethod',
        description: `Update the HTTP method for a specific tab. The tabId must correspond to an existing tab. The method should be a valid HTTP method string (e.g., GET, POST, PUT, DELETE).`,

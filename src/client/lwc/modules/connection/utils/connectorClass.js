...
import { getConfiguration,extractName,normalizeConfiguration,extractConfigurationValuesFromConnection } from './utils';
...
export class Connector {
    ...
    async _enrichConnector(){
        try{
            ...
            let latestVersion = Array.isArray(versions)? versions.sort((a, b) => b.version.localeCompare(a.version))[0]: undefined;
            ...
            if(sessionSettings && sessionSettings[CACHE_SESSION_CONFIG.API_VERSION.key]){
                currentVersion = versions.find(x => x.version === sessionSettings[CACHE_SESSION_CONFIG.API_VERSION.key]) || latestVersion;
            }
     ...
     */
    static async createConnector({ alias, connection, configuration, redirectUrl, credentialType,isEnrichDisabled }) {
        if (!configuration) {
        ...
        if (connection) {
            Object.assign(configuration,
                extractConfigurationValuesFromConnection(connection)
            );
        }

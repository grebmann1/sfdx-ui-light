...
export default class App extends LightningElement {
    ...
    shellChange({ shell }) {
        ...
        // Open Application
        if (shell.openTarget) {
            this.handleApplicationSelection(shell.openTarget);
            // Clear after consumption to match legacy behavior (state persists until next action)
            // We'll clear it after handling to prevent re-triggering
            store.dispatch(SHELL.reduxSlice.actions.clearOpen());
        }

        // Toggle Menu
        if (isNotUndefinedOrNull(shell.menuDisplayed)) {
            this.isMenuHidden = !shell.menuDisplayed;
        }
        if (isNotUndefinedOrNull(shell.menuExpanded)) {
            this.isMenuCollapsed = !shell.menuExpanded;
        }

        // Toggle Agent Chat
        if (isNotUndefinedOrNull(shell.agentChatExpanded)) {
            this.isAgentChatExpanded = shell.agentChatExpanded;
        }

        // Redirect
        if (shell.redirectTo) {
            this.handleRedirection({ redirectTo: shell.redirectTo });

...
function getSuggestions(monaco, model, position) {
    ...
    let completions = getSnippets(monaco, snippetRange);
...
function getSnippets(monaco, range) {
    const SNIPPETS = [
        {
...
export const ROOT_COMPLETION_TYPES = [
    'String',
    'Set',
    'Schema',
    'Schedulable',
...
const caseInsensitiveKeyMap = ROOT_COMPLETION_TYPES.reduce((output, item) => {
    output[item.toLowerCase()] = item;
    return output;
}, {});

export function getProperCasedCompletionKey(key) {
    return caseInsensitiveKeyMap[key.toLowerCase()];
}

export const APEX_COMPLETIONS = {
    String: {
        constructors: [],
        methods: [
            {

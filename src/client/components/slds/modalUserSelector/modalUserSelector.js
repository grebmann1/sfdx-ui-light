...
export default class ModalUserSelector extends LightningModal {
    ...
    loadTable = async () => {
		...
		let colModel = [
			...
			{ title: 'First Name', vertAlign: "middle", field: 'FirstName',resizable: true,width: 150,
				cellClick:function(e, cell) {
					cell.getRow().toggleSelect();
				}
			},
			{ title: 'Last Name', vertAlign: "middle", field: 'LastName',resizable: true,width: 170,
				cellClick:function(e, cell) {
					cell.getRow().toggleSelect();
				}
			},
			{ title: 'Email', vertAlign: "middle", field: 'Email',resizable: true,
				cellClick:function(e, cell) {
					cell.getRow().toggleSelect();
				}
			},
            { title: 'Username', vertAlign: "middle", field: 'Username',resizable: true,
				cellClick:function(e, cell) {
					cell.getRow().toggleSelect();
				}
			},
            { title: 'Profile', vertAlign: "middle", field: '_profile',resizable: true,width: 170,
				cellClick:function(e, cell) {
					cell.getRow().toggleSelect();
				}
			},
            { title: 'Active', vertAlign: "middle", hozAlign:"center", field: 'IsActive',resizable: true,width: 75,
				cellClick:function(e, cell) {
					cell.getRow().toggleSelect();
				},
        ...
        this.tableInstance.on("tableBuilt", () =>{
            ...
            this.tableInstance.selectRow(this.tableInstance.getRows().filter(
                row => this.selected.includes(row.getData().Id)
            ));
        })
    }
    ...
    loadUsers = async() => {
        this.isLoading = true;
        let query = this.conn.query(`SELECT id,Username,Name,Email,FirstName,LastName,Profile.Name,IsActive FROM User`);
        let records = await query.run({ responseTarget:'Records',autoFetch : true, maxFetch : 500 }) || [];

        this.records = records.map(x => ({...x,...{
